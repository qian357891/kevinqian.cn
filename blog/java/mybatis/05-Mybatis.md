...
* 编写测试用例
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729112907106.png)
...
* 安装 MyBatisX 插件
  ...
  * 安装方式
    ...
    ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729113304743.png)

    > 注意：安装完毕后需要重启IDEA

  * 插件效果

    ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729164450524.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729165724838.png)
...
* 编写接口方法：Mapper接口
  ...
  * 参数：无
    ...
    ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729171208737.png)

  * 结果：`List<Brand>`

    我们会将查询出来的每一条数据封装成一个 `Brand` 对象，而多条数据封装多个 `Brand` 对象，需要将这些对象封装到List集合中返回。

    ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729171146911.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729173210433.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729180118287.png)
...
* 编写接口方法：Mapper接口
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729180604529.png)
...
* 编写SQL语句：SQL映射文件
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729180709318.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729182223137.png)
...
* ${} ：拼接SQL。底层使用的是 `Statement`，会存在SQL注入问题。如下图将 映射配置文件中的 #{} 替换成 ${} 来看效果
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729184156019.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729184756094.png)
...
* 转义字符
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729185128686.png)

* `<![CDATA[内容]]>`

  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729185030318.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729204458815.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729213613029.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729214548756.png)

### 1.6  添加数据

![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729214917317.png)

如上图是我们平时在添加数据时展示的页面，而我们在该页面输入想要的数据后添加 `提交` 按钮，就会将这些数据添加到数据库中。接下来我们就来实现添加数据的操作。

* 编写接口方法

  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729215351651.png)

  参数：除了id之外的所有的数据。id对应的是表中主键值，而主键我们是 ==自动增长== 生成的。

* 编写SQL语句

  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729215537167.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729221207962.png)

订单数据存储在订单表中，订单项存储在订单项表中。

* 添加订单数据

  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729221049462.png)

* 添加订单项数据，订单项中需要设置所属订单的id

  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729221058898.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729222642700.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210729225713894.png)
...
* 运行代码结果如下
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210805230303461.png)
...
* 将之前案例中 `UserMapper.xml` 中的 根据id查询数据 的 `statement` 注释掉
  ...
  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210805235229938.png)

* 在 `UserMapper` 接口的 `selectById` 方法上添加注解

  ![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210805235405070.png)
...
![](https://qiankun825.oss-cn-hangzhou.aliyuncs.com/img/image-20210805234842497.png)
